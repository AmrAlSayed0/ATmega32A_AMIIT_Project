    ...
    if ( uartCnfg != NULL_PTR )
    {
        ...
        if ( ubbrResult == UART_OK )
        {
            ...
            switch ( uartCnfg->transmissionMode )
            {
                ...
                break;
                case UART_TRANS_SYNC :
                DIO_setDirection ( XCK_PORT , XCK_PIN , DIRECTION_OUTPUT );
                CLEAR_BIT ( ucsraValue , U2X );
                ...
                if ( opResult == UART_OK )
                {
                    ...
                    if ( opResult == UART_OK )
                    {
                        switch ( uartCnfg->dataFrameSize )
                        {
                            ...
                            REPLACE_BITS ( ucsrcValue , ( 0x00 << UCSZ1 ) | ( 0x00 << UCSZ0 ) , BIT_MASK2 ( UCSZ1 , UCSZ0 ) );
                            CLEAR_BIT ( ucsrbValue , UCSZ2 );
                            break;
                            case UART_DATA_SIZE_6_BIT :
                            REPLACE_BITS ( ucsrcValue , ( 0x00 << UCSZ1 ) | ( 0x01 << UCSZ0 ) , BIT_MASK2 ( UCSZ1 , UCSZ0 ) );
                            CLEAR_BIT ( ucsrbValue , UCSZ2 );
                            break;
                            case UART_DATA_SIZE_7_BIT :
                            REPLACE_BITS ( ucsrcValue , ( 0x01 << UCSZ1 ) | ( 0x00 << UCSZ0 ) , BIT_MASK2 ( UCSZ1 , UCSZ0 ) );
                            CLEAR_BIT ( ucsrbValue , UCSZ2 );
                            break;
                            case UART_DATA_SIZE_8_BIT :
                            REPLACE_BITS ( ucsrcValue , ( 0x01 << UCSZ1 ) | ( 0x01 << UCSZ0 ) , BIT_MASK2 ( UCSZ1 , UCSZ0 ) );
                            CLEAR_BIT ( ucsrbValue , UCSZ2 );
                            break;
                        ...
                        if ( opResult == UART_OK )
                        {
                            ...
                            {
                                case UART_PARITY_NO :
                                REPLACE_BITS ( ucsrcValue , ( 0x00 << UPM1 ) | ( 0x00 << UPM0 ) , BIT_MASK2 ( UPM1 , UPM0 ) );
                                break;
                                case UART_PARITY_EVEN :
                                REPLACE_BITS ( ucsrcValue , ( 0x01 << UPM1 ) | ( 0x00 << UPM0 ) , BIT_MASK2 ( UPM1 , UPM0 ) );
                                break;
                                case UART_PARITY_ODD :
                                REPLACE_BITS ( ucsrcValue , ( 0x01 << UPM1 ) | ( 0x01 << UPM0 ) , BIT_MASK2 ( UPM1 , UPM0 ) );
                                break;
                                default:
...
UART_STD_ERR_t UART_setTransmissionMode ( UART_TRANS_MODE_t transmissionMode )
{
    ...
    switch ( transmissionMode )
    {
        case UART_TRANS_ASYNC :
            ucsrcValue = UBRRH;
            ucsrcValue = UCSRC;
            REPLACE_BITS ( ucsrcValue , ( 0x01 << URSEL ) | ( 0x00 << UMSEL ) , BIT_MASK2 ( URSEL , UMSEL ) );
            UCSRC = ucsrcValue;
            break;
        case UART_TRANS_SYNC :
            ucsrcValue = UBRRH;
            ucsrcValue = UCSRC;
            REPLACE_BITS ( ucsrcValue , ( 0x01 << URSEL ) | ( 0x01 << UMSEL ) , BIT_MASK2 ( URSEL , UMSEL ) );
            UCSRC = ucsrcValue;
            break;
        default:
            opResult = UART_ERR_TRANS_MODE;
...
UART_STD_ERR_t UART_setClckPolarity ( UART_CLCK_POLARITY_t clckPolarity )
{
    ...
    switch ( clckPolarity )
    {
        case UART_CLCK_POLARITY_RISING_FALLING :
            ucsrcValue = UBRRH;
            ucsrcValue = UCSRC;
            REPLACE_BITS ( ucsrcValue , ( 0x01 << URSEL ) | ( 0x00 << UCPOL ) , BIT_MASK2 ( URSEL , UCPOL ) );
            UCSRC = ucsrcValue;
            break;
        case UART_CLCK_POLARITY_FALLING_RISING :
            ucsrcValue = UBRRH;
            ucsrcValue = UCSRC;
            REPLACE_BITS ( ucsrcValue , ( 0x01 << URSEL ) | ( 0x01 << UCPOL ) , BIT_MASK2 ( URSEL , UCPOL ) );
            UCSRC = ucsrcValue;
            break;
        case UART_CLCK_POLARITY_NOT_CARE :
            break;
...
UART_STD_ERR_t UART_setDataFrameSize ( UART_DATA_SIZE_t dataFrameSize )
{
    ...
    switch ( dataFrameSize )
    {
        case UART_DATA_SIZE_5_BIT :
            ...
            ucsrcValue = UBRRH;
            ucsrcValue = UCSRC;
            REPLACE_BITS ( ucsrcValue , ( 0x01 << URSEL ) | ( 0x00 << UCSZ1 ) | ( 0x00 << UCSZ0 ) , BIT_MASK3 ( URSEL , UCSZ1 , UCSZ0 ) );
            UCSRC = ucsrcValue;
            break;
        case UART_DATA_SIZE_6_BIT :
            CLEAR_BIT ( UCSRB , UCSZ2 );
            ucsrcValue = UBRRH;
            ucsrcValue = UCSRC;
            REPLACE_BITS ( ucsrcValue , ( 0x01 << URSEL ) | ( 0x00 << UCSZ1 ) | ( 0x01 << UCSZ0 ) , BIT_MASK3 ( URSEL , UCSZ1 , UCSZ0 ) );
            UCSRC = ucsrcValue;
            break;
        case UART_DATA_SIZE_7_BIT :
            CLEAR_BIT ( UCSRB , UCSZ2 );
            ucsrcValue = UBRRH;
            ucsrcValue = UCSRC;
            REPLACE_BITS ( ucsrcValue , ( 0x01 << URSEL ) | ( 0x01 << UCSZ1 ) | ( 0x00 << UCSZ0 ) , BIT_MASK3 ( URSEL , UCSZ1 , UCSZ0 ) );
            UCSRC = ucsrcValue;
            break;
        case UART_DATA_SIZE_8_BIT :
            CLEAR_BIT ( UCSRB , UCSZ2 );
            ucsrcValue = UBRRH;
            ucsrcValue = UCSRC;
            REPLACE_BITS ( ucsrcValue , ( 0x01 << URSEL ) | ( 0x01 << UCSZ1 ) | ( 0x01 << UCSZ0 ) , BIT_MASK3 ( URSEL , UCSZ1 , UCSZ0 ) );
            UCSRC = ucsrcValue;
            break;
        case UART_DATA_SIZE_9_BIT :
            SET_BIT ( UCSRB , UCSZ2 );
...
UART_STD_ERR_t UART_setParitySize ( UART_PARITY_t parity )
{
    ...
    switch ( parity )
    {
        case UART_PARITY_NO :
            ucsrcValue = UBRRH;
            ucsrcValue = UCSRC;
            REPLACE_BITS ( ucsrcValue , ( 0x01 << URSEL ) | ( 0x00 << UPM1 ) | ( 0x00 << UPM0 ) , BIT_MASK3 ( URSEL , UPM1 , UPM0 ) );
            UCSRC = ucsrcValue;
            break;
        case UART_PARITY_EVEN :
            ucsrcValue = UBRRH;
            ucsrcValue = UCSRC;
            REPLACE_BITS ( ucsrcValue , ( 0x01 << URSEL ) | ( 0x01 << UPM1 ) | ( 0x00 << UPM0 ) , BIT_MASK3 ( URSEL , UPM1 , UPM0 ) );
            UCSRC = ucsrcValue;
            break;
        case UART_PARITY_ODD :
            ucsrcValue = UBRRH;
            ucsrcValue = UCSRC;
            REPLACE_BITS ( ucsrcValue , ( 0x01 << URSEL ) | ( 0x01 << UPM1 ) | ( 0x01 << UPM0 ) , BIT_MASK3 ( URSEL , UPM1 , UPM0 ) );
            UCSRC = ucsrcValue;
            break;
        default:
            opResult = UART_ERR_PARITY;
...
void UART_transmit ( uint16_t data )
{
    ...
    while ( READ_BIT ( UCSRA , UDRE ) == ( uint8_t ) BIT_STATE_CLEARED )
    {
    }
    ...
    if ( is9BitDataFrame )
    {
        if ( READ_BIT ( data , 8 ) == ( uint8_t ) BIT_STATE_SET )
        {
            SET_BIT ( UCSRB , TXB8 );
        }
        else
...
uint16_t UART_receive ( void )
{
    ...
    while ( READ_BIT ( UCSRA , RXC ) == ( uint8_t ) BIT_STATE_CLEARED )
    {
    }
...
void UART_flush ( void )
{
    ...
    while ( READ_BIT ( UCSRA , RXC ) == ( uint8_t ) BIT_STATE_SET )
    {
        dummy = UDR;
    }
}
